...
new
Exception('Nette Framework requires PHP 5.2.0 or newer.');}error_reporting(E_ALL|E_STRICT);@set_magic_quotes_runtime(FALSE);@iconv_set_encoding('internal_encoding','UTF-8');extension_loaded('mbstring')&&mb_internal_encoding('UTF-8');umask(0);@header('X-Powered-By: Nette Framework');@header('Content-Type: text/html; charset=utf-8');empty($GLOBALS[0])&&$GLOBALS[0]=array();define('NETTE',TRUE);define('NETTE_DIR',dirname(__FILE__));define('NETTE_VERSION_ID',20018);define('NETTE_PACKAGE','PHP 5.2 prefixed');interface
IBarPanel{function
getTab();function
getPanel();}interface
IPresenter{function
...
htmlspecialchars($title),($exception->getCode()?' #'.$exception->getCode():'')?></h1>
			...
			<p><?php echo
htmlspecialchars($exception->getMessage())?> <a href="https://www.google.com/search?sourceid=nette&amp;q=<?php echo
...
addProtection($message=NULL,$timeout=NULL){$session=$this->getSession()->getSection('Nette.Forms.Form/CSRF');$key="key$timeout";if(isset($session->$key)){$token=$session->$key;}else{$session->$key=$token=NStrings::random();}$session->setExpiration($timeout,$key);$token=base64_encode($token^$this->getSession()->getId());$this[self::PROTECTOR_ID]=new
...
new
InvalidStateException("Cannot load node number $nodeId.");}++$i;continue;}do{$link=$data[$i];if(isset($this->deletedLinks[$link])){continue;}elseif(!isset($node[$link])){if(self::$debug){throw
new
...
setDebugMode($value=''){$this->parameters['debugMode']=is_string($value)||is_array($value)?self::detectDebugMode($value):(bool)$value;$this->parameters['productionMode']=!$this->parameters['debugMode'];return$this;}function
...
NCallback($factory);try{$reflection=$factory->toReflection();}catch(ReflectionException$e){}if(isset($e)||!$factory->isCallable()){throw
new
NServiceCreationException("Factory '$factory' used in service '$name' is not callable.");}$def->class=preg_replace('#[|\s].*#','',$reflection->getAnnotation('return'));if($def->class&&!class_exists($def->class)&&$def->class[0]!=='\\'&&$reflection
instanceof
...
new
NServiceCreationException("Service of type {$class} needed by $method not found. Did you register it in configuration file?");}else{throw
new
NServiceCreationException("Class {$class} needed by $method not found. Check type hint and 'use' statements.");}}else{if($container
instanceof
...
static
function
append($s,&$offset=0){if($offset+strlen($s)>self::LINE_LENGTH){$offset=1;$s=self::EOL."\t".$s;}$offset+=strlen($s);return$s;}static
function
...
function
write($line,$expectedCode=NULL,$message=NULL){fwrite($this->connection,$line.NMail::EOL);if($expectedCode&&!in_array((int)$this->read(),(array)$expectedCode,TRUE)){throw
new
...
new
NJsonException(self::$messages[5],5);}$forceArray=(bool)($options&self::FORCE_ARRAY);if(!$forceArray&&preg_match('#(?<=[^\\\\]")\\\\u0000(?:[^"\\\\]|\\\\.)*+"\s*+:#',$json)){throw
new
NJsonException(self::$messages[JSON_ERROR_CTRL_CHAR]);}$value=json_decode($json,$forceArray);if($value===NULL&&$json!==''&&strcasecmp($json,'null')){$error=PHP_VERSION_ID>=50300?json_last_error():0;throw
new

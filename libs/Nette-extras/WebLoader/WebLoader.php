...
abstract class WebLoader extends NControl {
	...
	/** @var string */
	private $generatedFileNamePrefix = "l-";
...
';
	...
	public function getGeneratedFilename(array $files = null) {
		...
		foreach($files as $f){
			$p = explode(DIRECTORY_SEPARATOR, $f);
			$temp_file_name[] = substr( $p[count($p)-1], 0, 10);
		}
		return $this->generatedFileNamePrefix . implode("|", $temp_file_name)."|" . 
				
				substr(
						md5(
							implode("|", $files) . "|" . $this->getLastModified($files)
						),
				0,10). $this->generatedFileNameSuffix;
//		return $this->generatedFileNamePrefix . implode("|", $temp_file_name) . "|" . md5(
//			implode("|", $files) . "|" . $this->getLastModified($files)
//		) . $this->generatedFileNameSuffix;
	...
	public function getContent(array $files = null) {
		...
		foreach ($files as $file) {			
			list($tmp,$base_file_url) = explode(WWW_DIR, $file);
			$content .= '/* '.$base_file_url.' */
'.$this->loadFile($file).'
';
		}

    ...
    public function getRules() {
         $sql = dibi::query('
            SELECT
                ...
                FROM [gui_acl] a
                JOIN [gui_acl_roles] ro ON (a.role_id = ro.id)
                LEFT JOIN [gui_acl_resources] re ON (a.resource_id = re.id)
                LEFT JOIN [gui_acl_privileges] p ON (a.privilege_id = p.id)
                ORDER BY a.id ASC
        ');
